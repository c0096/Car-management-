    ...
    private static async Task ExecuteSchemaAsync(SqlConnection connection)
    {
        var commandText = """
            ...
            (
                ...
                WHERE name = 'FK_DeclarationAttachments_VehicleSaleDeclarations'
            )
            BEGIN
                ALTER TABLE dbo.DeclarationAttachments
                ADD CONSTRAINT FK_DeclarationAttachments_VehicleSaleDeclarations
                FOREIGN KEY (DeclarationId) REFERENCES dbo.VehicleSaleDeclarations(Id)
                ON DELETE CASCADE;
            END;
